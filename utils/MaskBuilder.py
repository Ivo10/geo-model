...
def get_train_num(smesh_path, node_path):
    with open(smesh_path, 'r') as file:
        line = file.readline()
        train_num = list(map(int, line.split()))[0]
...
def mask_builder():
    ...
    train_mask = torch.cat((torch.ones(train_num), torch.zeros(node_num - train_num))).bool()
    test_mask = torch.cat((torch.zeros(train_num), torch.ones(node_num - train_num))).bool()
